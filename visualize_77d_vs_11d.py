...
def load_intf(intf_key):
    ...
    data = raw[:, :].copy()
    return data, north, east, dx, dy, nlines, ncells
...
def draw(idx=None):
    ...
    # ── left: 11-day interferogram + polygons ─────────────────────────────────
    ...
    if key11 and _cache[key11][0] is not None:
        ...
        if not polygs_11.empty:
            polygs_11.boundary.plot(ax=ax1, color='white', linewidth=1.5, zorder=5)
        ax1.set_aspect('auto')
        ...
        ax1.set_title(f"11-day: {key11}", fontsize=11)
        ax1.set_xlabel("Lon"); ax1.set_ylabel("Lat")
    else:
        ax1.text(0.5, 0.5, f"No 11-day intf for {key77[:8]}", ha='center', va='center', transform=ax1.transAxes)
        ax1.set_title(f"11-day: {key11 or 'N/A'}", fontsize=11)

    # ── right: 77-day confidence map + polygons ───────────────────────────────
    ...
    if os.path.exists(pred_path):
        ...
        if not polygs_77.empty:
            polygs_77.boundary.plot(ax=ax2, color='white', linewidth=1.5, zorder=5)
        ax2.set_aspect('auto')
        ...
        ax2.set_title(f"77-day confidence: {key77}", fontsize=11)
        ax2.set_xlabel("Lon")
    else:
        ax2.text(0.5, 0.5, f"No confidence map found", ha='center', va='center', transform=ax2.transAxes)
        ax2.set_title(f"77-day: {key77}", fontsize=11)
