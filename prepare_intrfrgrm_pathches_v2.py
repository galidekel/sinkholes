...
if __name__ == '__main__':
    ...
    # Records the geographic lon/lat of column-0, row-0 of the patch array for each intf.
    # test_full_intf_v2.py reads this to position predictions back into lon/lat space.
    patch_coords_by_intf = {}
    ...
    for item in input_file_paths:
        ...
        # Alignment: crop each frame so its top-left aligns to a common lon/lat origin.
        # We capture x0_aligned, y0_aligned (the new top-left after cropping) so we can
        # record the exact geographic coordinate of patch column 0.
        if args.align_frames:
            if frame == 'North':
                data, mask, x0_aligned, y0_aligned, _ = crop_to_start_xy(data, mask, x0, y0, args.x0_N, 31.79)
            else:
                data, mask, x0_aligned, y0_aligned, _ = crop_to_start_xy(data, mask, x0, y0, args.x0_S, 31.44)
        else:
            x0_aligned, y0_aligned = x0, y0

        # Geographic longitude of patch array column 0 = x0_aligned + offset_x * dx.
        # This is the value test_full_intf_v2.py needs to convert pixel indices → lon/lat.
        x0_patch = x0_aligned + args.offset_x * dx
        y0_patch = y0_aligned  # patchify applies no y-offset

        logging.info(f'{intfrgrm_name}: x0_aligned={x0_aligned:.6f}, x0_patch={x0_patch:.6f}, y0_patch={y0_patch:.6f}')
        ...
        patch_coords_by_intf[intfrgrm_name] = {
            "x0_patch":   float(x0_patch),
            "y0_patch":   float(y0_patch),
            "dx":         float(dx),
            "dy":         float(dy),
            "frame":      frame,
    ...
    # ----- patch_coords.json -----
    # Consumed by test_full_intf_v2.py to position predictions in lon/lat space.
    coords_path = os.path.join(data_output_dir, "patch_coords.json")
    with open(coords_path, "w", encoding="utf-8") as f:
        json.dump(patch_coords_by_intf, f, indent=2)
    print("Wrote", coords_path)
